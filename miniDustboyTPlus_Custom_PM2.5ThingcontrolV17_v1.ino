...
void t6CheckTime() {
  ...
  if (connectWifi == false) {
    if (_epoch != 0 && millis() > 300000 && hour(_epoch + (millis()/1000)) == 0) {
...
void setup() {
  ...
  if (connectWifi == false) {
    ...
    getIP(serverIP,serverPort,retJson);
  }
  if (connectWifi == true) {
...
/*
  ...
  if (!wifiManager.autoConnect("@DustboyTPlus")) {
    Serial.println("failed to connect and hit timeout");
    //reset and try again, or maybe put it to deep sleep
    //    ESP.reset();
    delay(1000);
...
void splash() {
  ...
  for ( int i = 0; i < 170; i++)
  {
    tft.drawString("Waiting for WiFi Setup", xpos, 100, GFXFF);
...
void t3CallSendData() {
  ...
  if (connectWifi == false) {
    ...
    client.setInsecure();
    if (!client.connect("thingcontrol.io", 443)) {
      Serial.println("connection failed");
      return;
    }
    String req = "";
    req += "POST /api/v1/";
    req.concat(deviceToken);
    req += "/telemetry HTTP/1.1\r\n";
    req += "Host: thingcontrol.io\r\n";
